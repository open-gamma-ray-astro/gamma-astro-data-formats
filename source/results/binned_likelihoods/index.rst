...
:ref:`tscube` are two formats for serializing bin-by-bin likelihoods
to a FITS file.  A Likelihood SED stores the bin-by-bin likelihood
for a single source or test source position while a TSCube stores
bin-by-bin likelihoods for a grid of positions.
...
* ``NORM_SCAN`` -- ndim: 2, unit: None
    ...
    * Normalization scan values in each energy bin in units of the
      ...
      ``REF_EFLUX``, ``REF_FLUX``, ``REF_DFDE``, or ``REF_NPRED`` vectors to get the
...
within the ROI.  At each spatial bin this tool calculates the
...
a 2-dimensional map with the test source TS evaluated at each spatial
